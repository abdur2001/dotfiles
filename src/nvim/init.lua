  ...
  ---@type LazyPluginSpec
  {
    ...
    { -- Autoformat
      ...
      opts = {
        ...
        formatters = {
          ruff_format_unchained = function()
            return {
              command = 'ruff',
              args = {
                'format',
                -- '--force-exclude',  -- Prevents ruff from working in '_build/' dir.
                '--stdin-filename',
                '$FILENAME',
                '-',
              },
          ...
          ruff_organize_imports_unchained = function()
            return {
              command = 'ruff',
              args = {
                'check',
        ...
        formatters_by_ft = {
          lua = { 'stylua' },
          python = { 'ruff_format_unchained', 'ruff_organize_imports_unchained', 'trim_whitespace' },
    ...
    -- TODO Reenable after bugs fixed (likely >= v1.6.0)
    ...
    { 'lukas-reineke/indent-blankline.nvim', event = 'BufEnter', main = 'ibl', opts = {} },
    { 'tpope/vim-fugitive' },
    ...
    -- },
    -- TODO - make this work again, or something like it
    -- {
...
function ipdb_set_trace()
  ...
  local _, num = string.gsub(current_line, ' ', ' ')
  -- local lines = vim.api.nvim_buf_get_lines(0, )
  local new_lines = { '', '# fmt: off', 'import ipdb; ipdb.set_trace()', '# fmt: on', '' }
  for i, v in ipairs(new_lines) do
    -- new_lines[i] = " " * num .. v
    new_lines[i] = v
  end
  vim.api.nvim_buf_set_lines(0, current_row, current_row, true, new_lines)
end
...
function append_pragma_no_cover()
  local current_row = vim.api.nvim_win_get_cursor(0)[1]
  local current_line = vim.api.nvim_get_current_line()
  vim.api.nvim_buf_set_lines(0, current_row - 1, current_row, true, { current_line .. '  # pragma: no cover' })
end
vim.keymap.set('n', '<leader>ic', append_pragma_no_cover, { desc = 'Insert `  # pragma: no cover` at the end of the line' })

vim.keymap.set('n', '<leader>m', function()
  vim.cmd 'caddexpr expand("%") . ":" . line(".") . ": " . getline(".")'
end, { desc = 'Add the current line to the quickfix list' })

vim.keymap.set('n', '<leader>pp', function()
  vim.cmd 'cgetexpr system("make type-check | grep .py")'
end, { desc = 'Add the current line to the quickfix list' })
...
local function new_def_handler(err, result, ctx, config)
  original_def_handler(err, result, ctx, config)
  vim.notify 'called'

  if not err and result and (vim.islist(result) and #result > 0 or result.uri) then
    vim.cmd 'normal! zt'
  end
end

vim.lsp.handlers['textDocument/definition'] = new_def_handler
...
vim.keymap.set('n', 'gZ', function()
  ...
  vim.api.nvim_set_current_buf(buf)
  go_to_definition()
end, { desc = 'Open definition in new tab' })

local quick_surround_chars = {
  '[',
  ']',
  '(',
  ')',
...
for _, char in ipairs(quick_surround_chars) do
  vim.keymap.set('v', char, function()
    vim.cmd.normal { 'gza' .. char }
